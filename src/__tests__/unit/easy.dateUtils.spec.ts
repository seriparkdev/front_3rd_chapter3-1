...
describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    expect(getWeekDates(new Date('2024-10-02'))).toStrictEqual([
      new Date('2024-09-29'),
      new Date('2024-09-30'),
      new Date('2024-10-01'),
      new Date('2024-10-02'),
  ...
  it('주의 시작(일요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    expect(getWeekDates(new Date('2024-09-29'))).toStrictEqual([
      new Date('2024-09-29'),
      new Date('2024-09-30'),
      new Date('2024-10-01'),
      new Date('2024-10-02'),
  ...
  it('주의 끝(일요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    expect(getWeekDates(new Date('2024-10-06'))).toStrictEqual([
      new Date('2024-10-06'),
      new Date('2024-10-07'),
      new Date('2024-10-08'),
      new Date('2024-10-09'),
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연말)', () => {
    expect(getWeekDates(new Date('2024-12-30'))).toStrictEqual([
      new Date('2024-12-29'),
      new Date('2024-12-30'),
      new Date('2024-12-31'),
      new Date('2025-01-01'),
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연초)', () => {
    expect(getWeekDates(new Date('2025-01-01'))).toStrictEqual([
      new Date('2024-12-29'),
      new Date('2024-12-30'),
      new Date('2024-12-31'),
      new Date('2025-01-01'),
  ...
  it('윤년의 2월 29일을 포함한 주를 올바르게 처리한다', () => {
    expect(getWeekDates(new Date('2024-02-29'))).toStrictEqual([
      new Date('2024-02-25'),
      new Date('2024-02-26'),
      new Date('2024-02-27'),
      new Date('2024-02-28'),
  ...
  it('월의 마지막 날짜를 포함한 주를 올바르게 처리한다', () => {
    expect(getWeekDates(new Date('2024-05-31'))).toStrictEqual([
      new Date('2024-05-26'),
      new Date('2024-05-27'),
      new Date('2024-05-28'),
      new Date('2024-05-29'),
...
describe('getEventsForDay', () => {
  it('특정 날짜(1일)에 해당하는 이벤트만 정확히 반환한다', () => {
    expect(getEventsForDay(mockEvents, 1)).toStrictEqual([events[0]]);
  });

  it('해당 날짜에 이벤트가 없을 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(mockEvents, 3)).toStrictEqual([]);
  });
  ...
  it('날짜가 32일 이상인 경우 빈 배열을 반환한다', () => {
    expect(getEventsForDay(mockEvents, 32)).toStrictEqual([]);
    expect(getEventsForDay(mockEvents, 33)).toStrictEqual([]);
  });
});

describe('formatWeek', () => {
  it('월의 중간 날짜에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-10-15'))).toBe('2024년 10월 3주');
  });

  it('월의 첫 주에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-10-1'))).toBe('2024년 10월 1주');
  });

  it('월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-10-30'))).toBe('2024년 10월 5주');
  });
  ...
  it('평년 2월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2023-02-29'))).toBe('2023년 3월 1주');
  });
});
